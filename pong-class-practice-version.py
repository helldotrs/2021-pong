...
p1                  = Player(250, 50,   10, 90, K_w,   K_s,    color.WHITE, 5, 0)
p2                  = Player(250, 850,  10, 90, K_UP,  K_DOWN, color.WHITE, 5, 0)
...
while sysClass.running:
    ...
    #player 1 input
    if   ((keys[p1.key_up] or keys[p2.key_up])   and p1.y>0):
        p1.y    -= p1.paddle_s
    elif ((keys[p1.key_down] or keys[p2.key_down]) and p1.y + p1.paddle_h < my_screen.SCRN_H):
        p1.y    += p1.paddle_s


    #player 2 input
    if   ((keys[p2.key_up] or keys[p1.key_up])   and p2.y>0):
        p2.y    -= p2.paddle_s
    elif ((keys[p2.key_down] or keys[p1.key_down]) and p2.y + p2.paddle_h < my_screen.SCRN_H):
        p2.y    += p2.paddle_s
